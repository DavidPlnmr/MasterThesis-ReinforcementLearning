...
l'atterrissage, soit au crash, soit lors d'une sortie de la zone de simulation. Un
...
utilisant le sampler \textit{Tree-structured Parzen Estimator} (TPE). TPE est un algorithme bayésien qui modélise séparément la distribution des bons et des mauvais hyperparamètres, et dirige la recherche vers les régions prometteuses de l'espace de configuration \parencite{bergstraAlgorithmsHyperparameterOptimization2011}. Chaque configuration est évaluée sur \textbf{250\,000 pas de temps} d'entraînement, suivie d'une évaluation
déterministe sur \textbf{20 épisodes}. La métrique optimisée est la récompense moyenne obtenue lors de cette évaluation finale.

Ce budget de 250 000 steps, inférieur aux 500 000 steps retenus pour l'évaluation finale (Section \ref{sec:eval_finale}), résulte d'un compromis pragmatique : avec une cible de 150 essais par combinaison algorithme/environnement, un budget de tuning plus élevé aurait rendu la campagne d'optimisation computationnellement prohibitive. Ce choix n'est cependant pas neutre : il repose sur l'hypothèse implicite que les hyperparamètres optimaux à 250 000 steps restent valides à 500 000 steps.

\subsubsection{Élagage des essais non concluants}

Afin de réduire le coût computationnel, un pruner médian est
...
\begin{itemize}
  \item Les quinze premiers essais sont systématiquement menés à terme, sans élagage possible, afin de fournir au pruner une base de référence fiable avant d'activer le mécanisme.
  \item Au sein d'un essai, l'élagage ne peut s'activer qu'après 30\,000 steps, laissant le temps à l'agent d'amorcer sa convergence avant d'être jugé.
\end{itemize}
...
multicouches (\textit{MlpPolicy} dans Stable-Baselines3). L'espace
...
\textit{learning\_starts}
  & Nombre de pas avant le début des mises à jour.
...
\textit{train\_freq}
  ...
  & $\{1,\;4,\;8,\;16\}$ \\[4pt]
\textit{target\_update\_interval}
  & Nombre de mises à jour entre deux synchronisations du réseau cible. Le \textit{target network} stabilise
    l'apprentissage en fixant temporairement les valeurs cibles.
  & $\{100,\;250,\;500,\;1000\}$ \\[4pt]
\textit{exploration\_fraction}
  & Fraction du training durant laquelle $\varepsilon$ décroît de sa
    valeur initiale à sa valeur finale.
  & $[0.05,\;0.5]$ \\[4pt]
\textit{exploration\_final\_eps}
  & Valeur minimale de $\varepsilon$ après la phase d'exploration. Correspond au taux d'actions aléatoires résiduel
    en exploitation.
  & $[0.01,\;0.1]$ \\[4pt]
\textit{net\_arch}
...
\textit{n\_steps}
  & Nombre de pas collectés avant chaque mise à jour de la politique,
    \textit{rollout length}. Doit être supérieur
...
\textit{n\_epochs}
  & Nombre de passes sur les données collectées à chaque mise à jour Augmenter ce nombre améliore l'utilisation des données mais peut déstabiliser la politique.
  & $\{1,\;3,\;5,\;10,\;20\}$ \\[4pt]
\textit{gamma}
  & Facteur d'actualisation des récompenses futures.
  & $\{0.90,\ldots,0.999\}$ \\[4pt]
\textit{gae\_lambda}
  & Paramètre $\lambda$ du \textit{Generalized Advantage Estimation} (GAE). Contrôle le compromis biais-variance dans
    l'estimation de l'avantage.
  & $\{0.80,\ldots,1.0\}$ \\[4pt]
\textit{ent\_coef}
  & Coefficient de la prime d'entropie.
    Encourage l'exploration en pénalisant les politiques trop
    déterministes.
  & $[10^{-8},\;0.1]$ \\[4pt]
\textit{clip\_range}
  & Paramètre $\varepsilon$ de la contrainte PPO.
    Limite l'écart entre l'ancienne et la nouvelle politique lors des
    mises à jour.
  & $\{0.1,\;0.2,\;0.3,\;0.4\}$ \\[4pt]
\textit{max\_grad\_norm}
  & Norme maximale du gradient --- \textit{gradient clipping}. Prévient les mises à jour explosives et
    stabilise l'entraînement.
  & $[0.3,\;5.0]$ \\[4pt]
\textit{vf\_coef}
  & Coefficient de la perte du critique. Contrôle
    l'importance relative de l'erreur de prédiction de valeur par rapport
    à la perte de politique.
  & $[0.1,\;1.0]$ \\[4pt]
\textit{net\_arch}
  & Architecture du réseau MLP acteur-critique.
  & \textit{small} [64,64],\newline
    \textit{medium} [256,256],\newline
    \textit{large} [400,300] \\
\end{longtable}
...
\textit{learning\_starts}
  ...
  & $\{1000,\;5000,\;10000\}$ \\[4pt]
\textit{batch\_size}
  & Taille des mini-batchs pour les mises à jour.
  & $\{32,\;64,\;128,\;256,\;512\}$ \\[4pt]
\textit{gamma}
  & Facteur d'actualisation des récompenses futures.
  & $\{0.90,\ldots,0.999\}$ \\[4pt]
\textit{tau}
  & Taux de mise à jour \textit{soft} du réseau cible.
    À chaque pas~:
    $\theta_{\text{cible}} \leftarrow \tau\,\theta + (1-\tau)\,\theta_{\text{cible}}$.
  & $\{0.001,\;0.005,\;0.01,\;0.02,\;0.05\}$ \\[4pt]
\textit{ent\_coef}
  & Coefficient d'entropie géré automatiquement (\textit{auto}) : SAC
    ajuste ce coefficient à chaque pas de gradient pour maintenir une
    entropie cible $\mathcal{H}^* = -\dim(\mathcal{A})$, définie par
    \cite{haarnojaSoftActorCriticAlgorithms2019} comme l'opposé de la dimension de l'espace d'action.
  & \textit{auto} \\[4pt]
\textit{use\_sde}
  & Active la \textit{State-Dependent Exploration} (SDE). Le bruit d'exploration dépend de l'état courant
...
\textit{target\_update\_interval}
  ...
  & $\{1,\;2,\;4\}$ \\[4pt]
\textit{net\_arch}
  & Architecture des réseaux MLP acteur et critique.
  & \textit{small} [64,64],\newline
    \textit{medium} [256,256],\newline
    \textit{large} [400,300] \\
\end{longtable}
...
\textit{learning\_rate}           & $4.73 \times 10^{-4}$ \\
\textit{buffer\_size}             & $10^{4}$ \\
\textit{learning\_starts}         & $5\,000$ \\
\textit{batch\_size}              & $64$ \\
\textit{gamma}                    & $0.99$ \\
\textit{train\_freq}              & $1$ \\
\textit{target\_update\_interval} & $250$ \\
\textit{exploration\_fraction}    & $0.253$ \\
\textit{exploration\_final\_eps}  & $0.075$ \\
\textit{net\_arch}                & \textit{medium} $[256,\;256]$ \\
\end{longtable}
\endgroup
...
\textit{learning\_rate} & $2.37 \times 10^{-4}$ \\
\textit{n\_steps}       & $2\,048$ \\
\textit{batch\_size}    & $256$ \\
\textit{n\_epochs}      & $20$ \\
\textit{gamma}          & $0.995$ \\
\textit{gae\_lambda}    & $0.98$ \\
\textit{ent\_coef}      & $1.55 \times 10^{-2}$ \\
\textit{clip\_range}    & $0.4$ \\
\textit{max\_grad\_norm}& $2.13$ \\
\textit{vf\_coef}       & $0.889$ \\
\textit{net\_arch}      & \textit{large} $[400,\;300]$ \\
\end{longtable}
\endgroup
 
\medskip
 
La configuration PPO discrète se distingue par un seuil d'écrêtage $\epsilon$ de
...
Un seuil d'écrêtage élevé autorise des mises à jour de politique plus
importantes à chaque gradient step, tout en conservant la garantie de
stabilité propre à PPO grâce au mécanisme de clipping. Cette valeur, associée à vingt époques d'optimisations sur des rollouts de 2048 transitions, indique que PPO effectue $\lfloor 2048 / 256 \rfloor = 8$ mini-batches par époque, soit 160 mises à jour du réseau pour chaque rollout de 2048 transitions.
...
\textit{learning\_rate} & $6.42 \times 10^{-5}$ \\
\textit{n\_steps}       & $2\,048$ \\
\textit{batch\_size}    & $32$ \\
\textit{n\_epochs}      & $10$ \\
\textit{gamma}          & $0.995$ \\
\textit{gae\_lambda}    & $0.98$ \\
\textit{ent\_coef}      & $1.27 \times 10^{-4}$ \\
\textit{clip\_range}    & $0.4$ \\
\textit{max\_grad\_norm}& $1.65$ \\
\textit{vf\_coef}       & $0.105$ \\
\textit{net\_arch}      & \textit{large} $[400,\;300]$ \\
\end{longtable}
\endgroup
...
valeurs communes pour plusieurs hyperparamètres structurels (la longueur de rollout (2048 steps), le paramètre $\lambda$ du GAE ($0.98$), le seuil d'écrêtage $\epsilon$ ($0.4$) et l'architecture du réseau (large)), ce qui suggère que ces choix sont robustes à la nature de l'espace d'action sur cet environnement. En revanche, deux hyperparamètres diffèrent significativement entre les deux variantes : le taux d'apprentissage est quatre fois plus faible pour la variante continue ($6.42 \times 10^{-5}$ contre $2.37 \times 10^{-4}$), et le coefficient d'entropie est deux ordres de grandeur inférieur ($1.27 \times 10^{-4}$ contre $1.55 \times 10^{-2}$). Ces différences indiquent que l'optimisation d'une politique gaussienne en espace continu requiert des mises à jour plus prudentes et une pression d'exploration entropique moindre que dans le cas discret.
...
\textit{learning\_rate}           & $9.95 \times 10^{-4}$ \\
\textit{buffer\_size}             & $10^{5}$ \\
\textit{learning\_starts}         & $1\,000$ \\
\textit{batch\_size}              & $512$ \\
\textit{gamma}                    & $0.99$ \\
\textit{tau}                      & $0.05$ \\
\textit{ent\_coef}                & \textit{auto} \\
\textit{use\_sde}                 & \textit{True} \\
\textit{target\_update\_interval} & $2$ \\
\textit{net\_arch}                & \textit{medium} $[256,\;256]$ \\
\end{longtable}
\endgroup
...
le taux d'interpolation $\tau$ atteint sa valeur maximale testée ($0.05$), dix fois supérieure à la valeur standard de $0.005$ \parencite{haarnojaSoftActorCriticOffPolicy2018} : cette mise à jour rapide
...
confirme l'intérêt de la \textit{State-Dependent Exploration} pour les espaces
...
\begin{itemize}
  \item \textbf{PPO} : pendant l'entraînement, PPO échantillonne ses actions depuis une distribution de probabilités (gaussienne pour le continu, catégorielle pour le discret) afin d'explorer. En mode déterministe, l'action retenue est celle de probabilité maximale, sans aucun échantillonnage.

  \item \textbf{SAC} : de même, SAC paramétrise une distribution gaussienne et échantillonne lors de l'entraînement pour maximiser l'entropie. En mode déterministe, il retourne la moyenne de cette distribution, ce qui correspond à la politique la plus probable sans bruit d'exploration.

  \item \textbf{DQN} : le mode déterministe consiste à sélectionner systématiquement l'action de Q-valeur maximale (argmax), avec $\varepsilon = 0$. C'est déjà le comportement naturel de DQN lors de l'exploitation ; le mode déterministe ne change donc pas fondamentalement la politique évaluée.
\end{itemize}
...
rapportées sont la récompense moyenne et son écart-type inter-seeds (écart-type échantillonnal, $n-1$), les récompenses extrêmes observées, le taux de succès (fraction d'épisodes avec une récompense $\geq 200$) et le nombre de seeds
...
\caption{Résultats de l'évaluation multi-seeds (500\,000 steps,
         récompense moyenne sur 20 épisodes déterministes, écart-type échantillonnal $n-1$)}
...
SAC obtient la meilleure récompense moyenne ($233.44 \pm 52.50$) et le taux de succès le plus élevé ($79\%$, 4 seeds sur 5 résolues), ce qui va dans le sens d'une performance légèrement supérieure de l'algorithme à entropie maximale sur la variante continue -- un écart qui reste néanmoins du même ordre de grandeur que la variance inter-seeds observée, et qui ne permet donc pas de conclure à une supériorité statistiquement établie sur la seule base de ces 5 seeds.
...
en espace discret.\footnote{La valeur maximale de $278.76$ dans l'environnement discret (en évaluation multi-seeds) coïncide exactement avec le score Optuna de tuning pour cette configuration ; cette coïncidence a été vérifiée sur les données brutes et ne résulte pas d'une erreur de saisie.}
...
configuration retenue : un \textit{replay buffer} de $10^4$ transitions combiné à une fréquence de mise à jour maximale constituent une configuration agressive, optimisée
...
\caption{Distribution du temps de calcul (en minutes) pour chaque
...
de calcul inter-seeds. Cette métrique correspond au temps de processus observé pendant l’entraînement et constitue une approximation du coût temporel de convergence, sans coïncider exactement avec le temps de convergence algorithmique.
