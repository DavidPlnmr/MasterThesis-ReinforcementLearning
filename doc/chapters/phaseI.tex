...
\begin{itemize}
  \item \texttt{rollout/ep\_rew\_mean} et \texttt{rollout/ep\_rew\_std} :
        récompense moyenne et écart-type calculés sur une fenêtre glissante
        des derniers épisodes complétés (buffer circulaire interne à SB3,
        de taille \texttt{stats\_window\_size}),
  \item \texttt{rollout/ep\_len\_mean} : longueur moyenne des derniers
        épisodes,
...
Les hyperparamètres clés de chaque essai sont enregistrés dans le résumé
WandB (\texttt{run.summary}), permettant de générer des visualisations de
type \textit{Parallel Coordinates Plot} pour identifier les configurations
...
à \texttt{n\_epochs = 20} et \texttt{n\_steps = 2048}, indique que PPO effectue $\lfloor 2048 / 256 \rfloor = 8$ mini-batches\footnote{256 étant la taille d'un batch} par époque,
répétés sur \texttt{n\_epochs = 20} passes, soit 160 mises à jour du réseau
pour chaque rollout de 2048 transitions.
...
% -----------------------------------------------------------------------------
\subsubsection{Évaluation finale multi-seeds}
\label{sec:eval_finale}
% -----------------------------------------------------------------------------

% À compléter une fois evaluate.py terminé.
% Structure suggérée :
%
% \begin{table}
%   algo | env | mean ± std | min | max | success_rate | seeds résolues
% \end{table}
%
% Puis paragraphe d'analyse par algo (en s'appuyant sur les commentaires
% FIX ci-dessus), et synthèse comparative finale.