...
% Next needs to be written :
% The main goal of the game (score a goal in the opponent's net)
% Also talk about the reward sparse if i only include a reward for scoring a goal, and how that is not sufficient for learning. And include the refs for the reward shaping as a method to address the reward sparsity problem. Then talk about the different types of rewards that I used, and how I iteratively adjusted them to improve learning. Also talk about the specific rewards that I used to encourage specific behaviors, like the touch reward, the velocity reward, etc. Also talk about the challenges I faced with reward shaping, like the fact that some rewards could lead to undesired behaviors (reward hacking) and how I had to adjust those rewards to avoid that (AdvancedTouchReward, EnergyReward, KickoffReward).