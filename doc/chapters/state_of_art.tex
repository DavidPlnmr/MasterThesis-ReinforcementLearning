...
PPO est quasi-systématiquement combiné avec l'estimateur \textbf{GAE}
...
objectif \textbf{écrêtée} (\textit{Clipped Surrogate Objective}) qui limite
...
\texttt{n\_steps} transitions consécutives --- avant d'effectuer ses mises
à jour. Ce batch de données est exploité sur plusieurs passes d'optimisation
(\texttt{n\_epochs}), découpé en mini-batchs de taille \texttt{batch\_size}.
...
\textit{sample efficiency} structurellement limitée des méthodes
\textit{on-policy}~; néanmoins, le batch doit être intégralement renouvelé
après chaque cycle de mise à jour, car les données générées par la politique
modifiée ne seraient plus représentatives de la politique ayant produit les
estimations d'avantage \parencite{schulmanProximalPolicyOptimization2017}.
...
accumulées et le caractère aléatoire de sa politique, afin de rester
...
Ce choix correspond approximativement à l'entropie d'une distribution
uniforme sur un espace d'action normalisé, garantissant une exploration
suffisante sans sur-contraindre la politique.
...
Par défaut, SAC rééchantillonne un bruit gaussien indépendant à
chaque pas de temps, ce qui peut produire une exploration sans cohérence
temporelle. \textcite{raffinSmoothExplorationRobotic2021} proposent une
alternative, l'\textbf{State Dependent Exploration} (SDE) : plutôt que de
tirer un nouveau bruit à chaque step, SDE maintient un vecteur de bruit
$\varepsilon$ fixe pendant plusieurs steps consécutifs et apprend un mapping
$f(s, \varepsilon)$ dépendant de l'état courant. L'exploration devient ainsi
cohérente dans le temps --- l'agent explore des trajectoires entières plutôt
que des actions isolées --- et structurée par l'état, ce qui est
particulièrement adapté aux tâches de contrôle continu nécessitant des
séquences d'actions coordonnées. 
...
--- SAC adopte le principe du \textbf{double réseau critique} hérité de TD3 :
deux réseaux $Q_{\phi_1}$ et $Q_{\phi_2}$ sont entraînés indépendamment sur
les mêmes transitions, et la cible de mise à jour utilise systématiquement
le minimum des deux estimations :
...
\noindent où $\tau \ll 1$ (valeur standard : $0.005$) est le taux
