...
L'itération de politique (\textit{Policy Iteration}) est un processus qui permet de trouver la politique optimale en alternant deux étapes clés. La première consiste à évaluer la politique actuelle pour connaître sa valeur exacte (\textit{Policy Evaluation}). La seconde étape cherche ensuite à l'améliorer en choisissant systématiquement la meilleure action possible en fonction des résultats de l'évaluation précédente de manière \textit{greedy} (\textit{Policy Improvement}).

Ce processus d'évaluation suivi d'une amélioration est répété itérativement. La convergence vers la politique optimale ($\pi^*$) garantit alors systématiquement $V^{\pi^*} \geq V^\pi$ quel que soit l'état initial comparé \parencite{silverLecturesReinforcementLearning2015}.
...
Tant que la nouvelle politique générée diffère, ces opérations inter-cycles se relaient. Ce processus prend le nom d'\textbf{itération de politique modifiée} (\textit{modified policy iteration}) lorsque l'évaluation s'interrompt pour économiser du temps de calcul dès lors qu'un seuil minime $\epsilon$ est atteint entre deux évaluations.
...
alternative, la \textbf{State-Dependent Exploration} (SDE) : plutôt que de
